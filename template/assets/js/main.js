(function ($) {
  ...
  function updateStickyHeader() {
    ...
    var shouldShow = window.scrollY > 180 && footerTop > window.innerHeight;
  ...
  // ETAPA 04G: reference controls
  var multiCopyEnabled = false;
  var multiCopyValue = "";
  var $referenceStatus = $("<p>", {
    "class": "reference-copy-status",
    "role": "status",
    "aria-live": "polite"
  }).appendTo("body");

  function normalizeReferenceNumbers(value) {
    var normalized = String(value || "").trim();
    if (!/^\d+(,\d+)*$/.test(normalized)) {
      return [];
    }
    return normalized.split(",").filter(function (number, index, numbers) {
      return numbers.indexOf(number) === index;
    });
  }

  function fallbackReferenceCopy(value) {
  ...
  function writeReferenceClipboard(value) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      return navigator.clipboard.writeText(value).catch(function () {
        return fallbackReferenceCopy(value);
      });
    }
    return fallbackReferenceCopy(value);
  }

  function referenceTargetName($target, number) {
    if ($target.is("footer")) {
      return "footer";
    }
    if ($target.hasClass("hero-section")) {
      return "header";
    }
  ...
  function setMultiCopyState(enabled) {
    ...
    var selectedNumbers = normalizeReferenceNumbers(multiCopyValue);
    $(".reference-control--multi")
      .toggleClass("is-active", enabled)
      .attr("aria-pressed", String(enabled))
      .attr("aria-label", enabled ? "Disable multiple section-number copy" : "Enable multiple section-number copy");
    $(".reference-control--number").each(function () {
      var number = String($(this).attr("data-reference-number"));
      $(this).toggleClass("is-selected", enabled && selectedNumbers.indexOf(number) !== -1);
    });
    $referenceStatus.text(enabled ? "Multiple section-number copy enabled." : "Multiple section-number copy disabled.");
  }

  function showReferenceCopyResult($button, number, value, copied) {
    var priorTimer = $button.data("reference-copy-timer");
    if (priorTimer) {
      window.clearTimeout(priorTimer);
    }
    $button.toggleClass("is-copied", copied).toggleClass("is-copy-error", !copied);
    if (copied) {
      $button.html('<i class="fa-solid fa-check" aria-hidden="true"></i>');
      $button.attr("aria-label", "Copied " + value);
      $referenceStatus.text("Copied: " + value);
    } else {
      $button.text("!").attr("aria-label", "Copy failed for section " + number);
      $referenceStatus.text("Copy failed. Browser denied clipboard access.");
    }
    var timer = window.setTimeout(function () {
      $button.removeClass("is-copied is-copy-error").text(number).attr("aria-label", "Copy section number " + number);
    }, 1400);
    $button.data("reference-copy-timer", timer);
  }

  function renderReferenceControls() {
    $("[data-ref-cut-url][data-ref-original-url][data-reference-number]").each(function () {
      var $target = $(this);
      if ($target.children(".reference-controls").length) {
        return;
      }
      var number = String($target.attr("data-reference-number"));
      var targetName = referenceTargetName($target, number);
      var $controls = $("<div>", {
        "class": "reference-controls",
        "role": "group",
        "aria-label": "Reference controls for " + targetName
      });
      var $cut = $("<a>", {
        "class": "reference-control reference-control--cut",
        "href": $target.attr("data-ref-cut-url"),
        "target": "_blank",
        "rel": "noopener noreferrer",
        "aria-label": "Open cropped reference for " + targetName
      }).text("C");
      var $original = $("<a>", {
        "class": "reference-control reference-control--original",
        "href": $target.attr("data-ref-original-url"),
        "target": "_blank",
        "rel": "noopener noreferrer",
        "aria-label": "Open original reference for " + targetName
      }).text("O");
      var $number = $("<button>", {
        "class": "reference-control reference-control--number",
        "type": "button",
        "data-reference-number": number,
        "aria-label": "Copy section number " + number
      }).text(number);
      var $multi = $("<button>", {
        "class": "reference-control reference-control--multi",
        "type": "button",
        "aria-pressed": "false",
        "aria-label": "Enable multiple section-number copy"
      }).text("C+");
      $controls.append($cut, $original, $number, $multi);
      $target.addClass("has-reference-controls").append($controls);
    });
  }

  $(document).on("click", ".reference-control--multi", function () {
    setMultiCopyState(!multiCopyEnabled);
  });

  $(document).on("click", ".reference-control--number", function () {
    var $button = $(this);
    var number = String($button.attr("data-reference-number"));
    var value = number;
    if (multiCopyEnabled) {
      var numbers = normalizeReferenceNumbers(multiCopyValue);
      if (numbers.indexOf(number) === -1) {
        numbers.push(number);
      }
      multiCopyValue = numbers.join(",");
      value = multiCopyValue;
      $button.addClass("is-selected");
    }
    writeReferenceClipboard(value).then(function () {
      showReferenceCopyResult($button, number, value, true);
    }).catch(function () {
      showReferenceCopyResult($button, number, value, false);
    });
  });
